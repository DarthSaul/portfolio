...
- `src/utilities/projectsDev.js` — array of project objects (title, description, tech stack, links, image)
...
- `Work.jsx` — renders project cards by mapping over `projectsDev.js` data
- `ProjectCard.jsx` — individual project display card
- `About.jsx` / `SkillsTech.jsx` — bio and skills/technologies sections
- `Contact.jsx` — contact section

**Animations:**
- AOS (Animate On Scroll) — initialized in `App.jsx`, used via `data-aos` attributes for fade-in effects
- GSAP — installed but minimally used
- react-scroll — powers smooth anchor navigation between the `#home`, `#work`, and `#about` sections

**To add a project:** add an entry to `src/utilities/projectsDev.js`. The `Work` component will pick it up automatically.
